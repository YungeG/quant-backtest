...
class TushareProxyTradeCalendarMonthSourceBoundedRequestV2:
    exchange: str
    month: str
...
def _validate_rows(
    rows: list[list[object]], request: TushareProxyTradeCalendarMonthSourceBoundedRequestV2
) -> None:
...
def verify_tushare_proxy_trade_calendar_month_source_bounded_receipt_v2(
    ...
    if (
        receipt.get("type")
        != "tushare_proxy_trade_calendar_month_source_bounded_acquisition_receipt_v2"
    ...
    try:
        request = TushareProxyTradeCalendarMonthSourceBoundedRequestV2(
    ...
    expected_provenance = SourceSnapshotProvenance(
        ...
        source_key=(
            ...
            f"{receipt['transport_endpoint'].removeprefix('https://')}.trade_cal.szse.{request.month}"
        ),
    ...
    expected_provider_request_fields = {
        ...
        "response_byte_count", "response_sha256", "response_acquired_at_epoch_nanoseconds", "returned_row_count",
    ...
    if (
        ...
        or any(
            ...
            for field in ("attempts", "response_byte_count", "response_acquired_at_epoch_nanoseconds", "returned_row_count")
        )
        or provider_request["attempts"] not in range(1, 4)
        or provider_request.get("response_acquired_at_epoch_nanoseconds") != receipt.get("acquired_at_epoch_nanoseconds")
...
def acquire_tushare_proxy_trade_calendar_month_source_bounded_v2(
    request: TushareProxyTradeCalendarMonthSourceBoundedRequestV2,
    *,
    endpoint: str,
    output_dir: str | Path,
    ...
    if type(request) is not TushareProxyTradeCalendarMonthSourceBoundedRequestV2:
        raise AcquisitionError("request must be exact TushareProxyTradeCalendarMonthSourceBoundedRequestV2")
    ...
    snapshot = freeze_source_snapshot(
        ...
        provenance=SourceSnapshotProvenance(
            ...
            source_key=(
                "tushare.pro.via.xiaodefa.approved-proxy."
                f"{endpoint.removeprefix('https://')}.trade_cal.szse.{request.month}"
            ),
            license_ref="tushare.pro.terms",
            retention_policy_ref="backtest.acquisition.candidate",
        ),
    ...
    receipt: dict[str, object] = {
        "type": "tushare_proxy_trade_calendar_month_source_bounded_acquisition_receipt_v2",
        "schema_version": 2,
        "source_bounded": True,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        receipt = acquire_tushare_proxy_trade_calendar_month_source_bounded_v2(
            TushareProxyTradeCalendarMonthSourceBoundedRequestV2(_EXCHANGE, args.month),
