...
BASE = "25b8dd12a8a62530ce2467e13d1bd0b55b34b0cf"
MODULE = ROOT / "packages/market-bundle-builder/src/crypto_quant_bundle_builder/gree_historical_financial_statement_normalization_v1.py"
ALLOWED = {
...
def test_historical_normalization_is_pure_builder_only() -> None:
    ...
    root_export = (ROOT / "packages/market-bundle-builder/src/crypto_quant_bundle_builder/__init__.py").read_text(encoding="utf-8")
    assert "gree_historical_financial_statement_normalization_v1" not in root_export
    assert "GreeHistoricalFinancialStatementObservationRevisionV1" not in root_export


def test_pr7_base_tree_is_byte_and_mode_identical() -> None:
    ...
    for mode, blob, path in entries:
        ...
        current_hash = subprocess.check_output(["git", "hash-object", "--stdin"], cwd=ROOT, input=target).decode().strip()
        assert index_modes[path] == mode
        assert current_hash == blob


def test_historical_normalization_write_set_is_exact() -> None:
