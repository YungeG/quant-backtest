...
def request() -> TushareProxyTradeCalendarMonthSourceBoundedRequestV2:
    return TushareProxyTradeCalendarMonthSourceBoundedRequestV2("SZSE", "202402")
...
def acquire(tmp_path: Path, source: bytes) -> tuple[dict[str, object], FakePost]:
    ...
    receipt = acquire_tushare_proxy_trade_calendar_month_source_bounded_v2(
        request(), endpoint="https://fast.xiaodefa.cn", output_dir=tmp_path / "capture",
        post=post, clock=lambda: 1, sleep=lambda _: None,
    )
    return receipt, post
...
def test_receipt_verification_binds_raw_snapshot_worklist_and_identity(
    ...
    snapshot = freeze_source_snapshot(
        ...
        provenance=SourceSnapshotProvenance(
            "tushare.pro",
            "tushare.pro.via.xiaodefa.approved-proxy.fast.xiaodefa.cn.trade_cal.szse.202402",
            "tushare.pro.terms",
            "backtest.acquisition.candidate",
        ),
    ).snapshot
    assert snapshot is not None
    assert verify_tushare_proxy_trade_calendar_month_source_bounded_receipt_v2(
...
def test_request_requires_exact_szse_and_real_month(exchange: str, month: str) -> None:
    with pytest.raises(ValueError):
        TushareProxyTradeCalendarMonthSourceBoundedRequestV2(exchange, month)
...
class OrderedPost(FakePost):
    def __init__(self, source: bytes, events: list[str]) -> None:
        super().__init__(source)
        self.events = events

    def __call__(self, endpoint: str, body: dict[str, object], headers: dict[str, str]) -> tuple[int, bytes]:
        self.events.append("response")
        return super().__call__(endpoint, body, headers)


def test_clock_is_sampled_after_response_and_exactly_three_way_bound(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("TUSHARE_PROXY_TOKEN", TOKEN)
    events: list[str] = []
    receipt = acquire_tushare_proxy_trade_calendar_month_source_bounded_v2(request(), endpoint="https://fast.xiaodefa.cn", output_dir=tmp_path / "capture", post=OrderedPost(response(rows()), events), clock=lambda: (events.append("clock") or 17), sleep=lambda _: None)
...
def test_v2_verifier_rejects_v1_shape_and_timestamp_join(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("TUSHARE_PROXY_TOKEN", TOKEN)
    receipt, _ = acquire(tmp_path, response(rows()))
    raw = (tmp_path / "capture/response/trade-calendar.json").read_bytes()
    snapshot = freeze_source_snapshot(members=(RawSourceMember("response/trade-calendar.json", raw, "0644", 1, None),), provenance=SourceSnapshotProvenance("tushare.pro", "tushare.pro.via.xiaodefa.approved-proxy.fast.xiaodefa.cn.trade_cal.szse.202402", "tushare.pro.terms", "backtest.acquisition.candidate")).snapshot
    assert snapshot is not None
    v1_like = dict(receipt)
    v1_like["type"] = "tushare_proxy_trade_calendar_month_source_bounded_acquisition_receipt_v1"; v1_like["schema_version"] = 1
    v1_like["request"] = {**receipt["request"], "type": "tushare_proxy_trade_calendar_month_source_bounded_request_v1", "schema_version": 1}
    v1_request = dict(receipt["provider_requests"][0]); v1_request.pop("response_acquired_at_epoch_nanoseconds"); v1_like["provider_requests"] = [v1_request]
    v1_bytes = json.dumps(v1_like).encode()
    with pytest.raises(AcquisitionError, match="unexpected fields|type or schema"):
        verify_tushare_proxy_trade_calendar_month_source_bounded_receipt_v2(v1_bytes, snapshot, receipt_hash(v1_bytes))
    mismatched = dict(receipt); request_data = dict(receipt["provider_requests"][0]); request_data["response_acquired_at_epoch_nanoseconds"] = 2; mismatched["provider_requests"] = [request_data]
    mismatched_bytes = json.dumps(mismatched).encode()
    with pytest.raises(AcquisitionError, match="provider request"):
        verify_tushare_proxy_trade_calendar_month_source_bounded_receipt_v2(mismatched_bytes, snapshot, receipt_hash(mismatched_bytes))
