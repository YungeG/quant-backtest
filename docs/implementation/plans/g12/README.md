...
| [G12M-SOURCE-BOUNDED-QUALIFICATION-V1](g12m-source-bounded-qualification-v1.md) | Governance accepted; the exact Tushare A-share nominal boundary remains ready for provider-specific interface design. Binance funding-history v2 upstream evidence is accepted as post-hoc evidence, but causal Runtime qualification is blocked while its `available_time` remains the 2026 receipt instant | accepted G12I/G12K Tushare artifacts; accepted Binance funding-history v2 report plus target-effective provider availability authority before causal use | Runtime read-only qualification |
