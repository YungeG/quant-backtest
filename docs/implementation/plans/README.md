...
- [G07 durable rebuild proof v2](g07-durable-rebuild-proof-v2.md) — Matrix status `READY — DRP-00 CONTRACT_FROZEN`; six-node DAG with DRP-00 `ACCEPTED_H1` and DRP-01 sole Ready. H1 freezes nominal completed/analysis V2 contracts, stable pre-reopen no-fallback dispatch, local-durability versus static replay, cooperative recovery, and the Backtest-candidate → exact Platform pin → Matrix-row plus DAG-status governance fan-in.
