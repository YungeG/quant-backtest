...
status: PASSED
passed_commit: 373817b762fbe0d68b286577e0396107694cc9a1
...
test_results:
  focused_final: 49 passed
  broad_focused: 125 passed
  full_repository: 1842 passed
  import_boundaries: 112 files passed
  independent_review: NONE
artifact_hashes:
  source_record_hash: sha256:9b476b034dc596f296959ac817c7d797814b8e2b75498d5ec4c50da09e3d9f5e
  raw_bar_hash: sha256:7f0739da15fa80c99ccfdf9a1bdcca617376c20ed7b205c9b1e82b88f2759da0
  trace_hash: sha256:fc05a2fdc26d734bf6cd526eb22a149d36d5ab4022cfa2c67de0e807a7d6ee96
  execution_projection_hash: sha256:d283870ccf3042530bdc664aeb4f451b3723300fe3e22a59018b253c67b6bbb4
  valuation_projection_hash: sha256:2f9cf2a71c0f152e5377be0fdad798ec49cc411814b7cd3a372040286507a630
...
## 104G. G12L Tushare A-share Authority Acquisition v1
...
4. The public seam accepts `CashDevelopmentRequestIntent@1`, not a caller-constructed `BacktestRequest@1`. Backtest derives the immutable request only after producing its semantic spec, thereby retaining authority over profile keys, market/build/target/spec commitments, request hash, and semantic-run identity. It returns an opaque persisted `BacktestRequestRef`, executable `BacktestExecutionRequest@2`, and `PreparedBacktestExecution`, which is only an in-process immutable Python value containing the configured runtime; it has no wire type, schema version, Envelope, or persisted identity. No resolved request/case, plan, registry, dispatcher, builder, callback, path, or Platform type is exposed.
5. Real cash-provider terminal vectors use semantic inputs: missing MARKET capability for BLOCKED and additive `BacktestRuntime.run_with_cancellation()` propagation for CANCELLED while preserving existing `run()`. FAILED is not fabricated: production FAILED codes represent allocation/risk/rebalance/plan/accounting/internal-contract defects, and an ordinary exceeded risk limit yields an economically rejected zero target rather than `PORTFOLIO_RISK`. Platform approved real binding for COMPLETED/BLOCKED/CANCELLED while accepted repository evidence proves FAILED verification.
...
10. The Backtest owner approved the product. The provider input contains exactly nine external authorities: caller-owned base build manifest, one-instrument catalog, strategy ID, `StrategySleeveId`, initial cash, quantity lattice, decision `MarkObservation`, final `MarkObservation`, and order capabilities. A separate six-field request intent carries only experiment context, timeline window, account ID, reporting currency, and seed plus its fixed schema version. The decision observation is an exact zero-age/no-forward-fill `VALUATION` fact at decision time. The final observation precedes end-exclusive, is available no later than that boundary, and resolves there through provider-owned bounded forward-fill with max age exactly `end_exclusive - observed_at`, preserving the accepted RunEnd boundary rule. All request commitments, resolved marks, registry, policy, timeline, order, accounting, snapshot, identity, semantic, and runtime values remain Backtest-owned.
11. After deriving the fixed registry, preparation adds exactly three provider-owned `PROFILE_COMPONENT` `BuildArtifactRef` values for market/simulation/account using profile version, WHEEL/CLEAN identity, and derived content digests. Other base artifacts, libraries, lock/image identity, build key, and provenance remain exact. Exact existing refs are idempotent; a differing caller ref under any provider key fails before publication. The enriched manifest hash binds the request and v2 bundle, so Platform needs no private helper or profile digest knowledge.
12. The semantic ceiling is one target, one order, at most one full fill, fixed `targets`/`bars.open` streams, zero slippage, explicit no-fee assessment, no later cash flow, no settlement/financing/margin/liquidation/corporate action, and development grade only. Unsupported economics fail before Attempt creation.
