...
_PRICE_STREAM = "binance_usdm.tradifi.price_purpose.authority.koruusdt.v4"
_PRICE_EVENT = "binance_usdm_tradifi_price_purpose_binding_v4"
_PRICE_CAPABILITY = MarketBundleCapability("binance_usdm.price-purpose-streams", 1)
_ACCOUNT_STREAM = "binance_usdm.tradifi.account.authority.koruusdt.v4"
_ACCOUNT_EVENT = "account_financial_event"
_ACCOUNT_CAPABILITY = MarketBundleCapability("account.financial-event", 1)
_BUNDLE_PREFIX = "koru-tradifi-economics-development-v4-"
...
class KoruTradifiSourceProjectionContentIdentityV3:
    """Published V3 source-authority identity plus its exact source content binding."""
...
@dataclass(frozen=True, slots=True)
class KoruTradifiEconomicsTermsV4:
    ...
    funding_authority: Mapping[str, object] | None
    ...
    def __post_init__(self) -> None:
        ...
        if self.funding_authority is not None:
    ...
    def from_source_projection(
        cls, source_projection: BinanceUsdmKoruTradifiSourceProjectionResultV3, *, execution_account_id: str
        ...
        return cls(
            ...
            funding_authority=(
                ...
                if _FUNDING_STREAM in manifests
                else None
            ),
    ...
    def to_canonical_dict(self) -> dict[str, object]:
        return {
            "type": "koru_tradifi_economics_terms_v4",
...
@dataclass(frozen=True, slots=True)
class KoruTradifiEconomicsBundleRequestV4:
    """The sole publication input; store and root are operational, not identity."""

    source_projection: BinanceUsdmKoruTradifiSourceProjectionResultV3
    source_projection_content_identity: KoruTradifiSourceProjectionContentIdentityV3
    terms: KoruTradifiEconomicsTermsV4
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.source_projection_content_identity) is not KoruTradifiSourceProjectionContentIdentityV3:
            raise TypeError("source_projection_content_identity must be exact")
        if type(self.terms) is not KoruTradifiEconomicsTermsV4:
            raise TypeError("terms must be exact KoruTradifiEconomicsTermsV4")
    ...
    def to_canonical_dict(self) -> dict[str, object]:
        return {
            "type": "koru_tradifi_economics_bundle_request_v4",
            "schema_version": _SCHEMA_VERSION,
...
class KoruTradifiEconomicsBundleFailureCodeV4(str, Enum):
...
@dataclass(frozen=True, slots=True)
class KoruTradifiEconomicsBundleFailureV4:
    code: KoruTradifiEconomicsBundleFailureCodeV4
    subject: str

    def __post_init__(self) -> None:
        if type(self.code) is not KoruTradifiEconomicsBundleFailureCodeV4:
            raise TypeError("code must be exact KoruTradifiEconomicsBundleFailureCodeV4")
        _text(self.subject, "subject")

    def to_canonical_dict(self) -> dict[str, object]:
        return {
            "type": "koru_tradifi_economics_bundle_failure_v4",
            "schema_version": _SCHEMA_VERSION,
...
@dataclass(frozen=True, slots=True)
class KoruTradifiEconomicsBundleV4:
    request: KoruTradifiEconomicsBundleRequestV4
    ...
    def __post_init__(self) -> None:
        if (
            type(self.request) is not KoruTradifiEconomicsBundleRequestV4
    ...
    def _body(self) -> dict[str, object]:
        return {
            "type": "koru_tradifi_economics_bundle_v4",
            "schema_version": _SCHEMA_VERSION,
...
@dataclass(frozen=True, slots=True)
class KoruTradifiEconomicsBundleOutcomeV4:
    result: KoruTradifiEconomicsBundleV4 | None = None
    failure: KoruTradifiEconomicsBundleFailureV4 | None = None

    def __post_init__(self) -> None:
        if (self.result is None) == (self.failure is None):
            raise ValueError("outcome requires exactly one branch")
        if self.result is not None and type(self.result) is not KoruTradifiEconomicsBundleV4:
            raise TypeError("result must be exact KoruTradifiEconomicsBundleV4")
        if self.failure is not None and type(self.failure) is not KoruTradifiEconomicsBundleFailureV4:
            raise TypeError("failure must be exact KoruTradifiEconomicsBundleFailureV4")


def _failed(code: KoruTradifiEconomicsBundleFailureCodeV4, subject: str) -> KoruTradifiEconomicsBundleOutcomeV4:
    return KoruTradifiEconomicsBundleOutcomeV4(
        failure=KoruTradifiEconomicsBundleFailureV4(code, subject)
    )


def _validate_terms_against_source(
    terms: KoruTradifiEconomicsTermsV4,
    source: BinanceUsdmKoruTradifiSourceProjectionResultV3,
...
def _validate_source(
    request: KoruTradifiEconomicsBundleRequestV4,
) -> tuple[BinanceUsdmKoruTradifiSourceProjectionResultV3, ArtifactEnvelope]:
    ...
    if source is None:
        raise ValueError("source_projection")
    identity = request.source_projection_content_identity
    authority, authority_ref = create_binance_usdm_koru_tradifi_source_projection_authority_v3(source)
    if (
        identity.source_projection_authority_ref != authority_ref
        or identity.source_projection_authority_content_hash != authority.content_hash
        or identity.source_fragment_digest != source.fragment_digest
        or identity.source_projection_request_hash != source.request.request_hash
    ):
        raise ValueError("source_identity")
    source_profile, source_profile_ref = build_binance_usdm_koru_source_profile_authority_v3(source)
...
def _event(
    *, stream_key: str, event_type: str, capability: MarketBundleCapability,
    instant: UtcInstant, phase: TimelinePhase, payload: dict[str, object], instrument: InstrumentId | None,
) -> MarketEvent:
...
def _assemble(
    ...
    price_payload = {
        ...
        "source_projection_content_identity": _wire(request.source_projection_content_identity),
...
def publish_koru_tradifi_economics_bundle_v4(
    request: KoruTradifiEconomicsBundleRequestV4,
) -> KoruTradifiEconomicsBundleOutcomeV4:
    """Publish then locally reopen a target-free economics Market Bundle."""
    if type(request) is not KoruTradifiEconomicsBundleRequestV4:
        return _failed(KoruTradifiEconomicsBundleFailureCodeV4.INVALID_REQUEST, "request")
    try:
        source, source_profile = _validate_source(request)
    except (AttributeError, KeyError, TypeError, ValueError):
        return _failed(KoruTradifiEconomicsBundleFailureCodeV4.SOURCE_PROJECTION_INVALID, "source_projection")
    try:
        ...
        return _failed(KoruTradifiEconomicsBundleFailureCodeV4.TERMS_INVALID, "terms")
    artifacts = (source_profile, source.xkrx_calendar, source.arcx_calendar, source.post_adjustment_unit_regime)
    try:
        refs = _publish_artifacts(request.artifact_store, artifacts)
    except (AttributeError, TypeError):
        return _failed(KoruTradifiEconomicsBundleFailureCodeV4.ARTIFACT_STORE_INVALID, "artifact_store")
    except (KeyError, ValueError):
        return _failed(KoruTradifiEconomicsBundleFailureCodeV4.ARTIFACT_PUBLICATION_FAILED, "artifact_readback")
    outcome = LocalMarketBundleRepository(
        ...
        retention_policy_ref="koru-tradifi-economics-v4",
    )
    if outcome.result is None or outcome.result.bundle_ref != bundle_ref:
        return _failed(KoruTradifiEconomicsBundleFailureCodeV4.MARKET_PUBLICATION_FAILED, "market_bundle")
    try:
        ...
        return _failed(KoruTradifiEconomicsBundleFailureCodeV4.REOPEN_FAILED, "market_bundle")
    try:
        return KoruTradifiEconomicsBundleOutcomeV4(
            result=KoruTradifiEconomicsBundleV4(
                request=request,
        ...
        return _failed(KoruTradifiEconomicsBundleFailureCodeV4.RESULT_INVALID, "result")
