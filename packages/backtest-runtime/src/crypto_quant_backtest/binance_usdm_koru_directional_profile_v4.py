"""Sealed V4 KORU target authority reconstructed from a target overlay."""
...
_ECONOMICS_PREFIX = "koru-tradifi-economics-development-v4-"
...
class KoruDirectionalV4StrategyAuthority:
    """The sole V4 strategy identity admitted from a target overlay."""
    ...
    scope_digest: str
    source_projection_authority_ref: domain.ArtifactRef
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            or (self.compiler_result_ref.artifact_type, self.compiler_result_ref.schema_version) != ("koru_directional_target_compile_result", 2)
            or self.compiler_result_ref != domain.ArtifactRef("koru_directional_target_compile_result", 2, self.compiler_result_digest)
            ...
            or (self.source_projection_authority_ref.artifact_type, self.source_projection_authority_ref.schema_version) != ("binance_usdm_koru_tradifi_source_projection_authority_v3", 3)
            or self.source_projection_authority_ref.content_hash != self.source_projection_authority_content_hash
            or (self.source_profile_authority_ref.artifact_type, self.source_profile_authority_ref.schema_version) != ("binance_usdm_koru_source_profile_authority", 3)
        ):
            raise ValueError("authority_refs")
        for name in ("compiler_result_digest", "recipe_digest", "scope_digest", "source_projection_authority_content_hash", "source_fragment_digest"):
...
@dataclass(frozen=True, slots=True)
class BinanceUsdmKoruDirectionalTargetConsumptionV4:
    strategy_ref: domain.ArtifactRef
    parameter_ref: domain.ArtifactRef
    strategy_id: str
    sleeve_id: str
...
class BinanceUsdmKoruDirectionalPlannerV4:
    @staticmethod
    def target(authority: KoruDirectionalV4StrategyAuthority) -> BinanceUsdmKoruDirectionalTargetConsumptionV4:
        if type(authority) is not KoruDirectionalV4StrategyAuthority:
            raise TypeError("authority")
        return BinanceUsdmKoruDirectionalTargetConsumptionV4(
...
def _overlay_payload(reader: MarketBundleReader, manifest: MarketBundleManifest, bundle_ref: MarketBundleRef) -> Mapping[str, object]:
    ...
    required = {
        ...
        "instrument_catalog_hash", "source_fragment_digest", "source_projection_request_hash",
        "source_projection_authority_ref", "source_projection_authority_content_hash",
        "source_profile_authority_envelope", "source_profile_authority_ref",
        "aggregate_trade_boundary_index_request_hash", "aggregate_trade_boundary_index_result_digest",
        "aggregate_trade_streamed_reconstruction_digest", "aggregate_trade_capture_final_evidence",
        "compiler_result_ref", "compiler_result_digest", "scope_ref", "scope_digest", "scope",
        "compiler_source_projection_authority_ref", "compiler_source_projection_authority_content_hash",
        "recipe", "recipe_ref", "recipe_digest", "strategy_ref", "strategy_id", "sleeve_id",
        "target_stream_key", "target_stream_digest", "target_stream_manifest", "target_events", "target_events_digest",
    }
    if len(events) != 1:
    ...
    if (
        event.event_type != _AUTHORITY_EVENT or event.stream_key != _AUTHORITY_STREAM
    ...
    if (
        not isinstance(economics_manifest, Mapping) or economics_manifest.get("type") != "market_bundle_manifest"
        ...
        or tuple(economics_manifest.get("coverage_start", {}).items()) != tuple(payload["coverage_start"].items())
        or tuple(economics_manifest.get("coverage_end_exclusive", {}).items()) != tuple(payload["coverage_end_exclusive"].items())
    ...
    if (
        not isinstance(economics_streams, tuple) or not economics_streams
        or economics_manifest.get("streams") != economics_streams
        or any(not isinstance(stream, Mapping) or type(stream.get("stream_key")) is not str for stream in economics_streams)
        or len({stream["stream_key"] for stream in economics_streams}) != len(economics_streams)
        or any(actual.get(stream["stream_key"]) != stream for stream in economics_streams)
        or set(actual) != {stream["stream_key"] for stream in economics_streams} | {_AUTHORITY_STREAM, target_key}
        or target_key == _AUTHORITY_STREAM
        or any(key.startswith(_V2_TARGET_PREFIX) or key in _FORBIDDEN_STREAMS for key in actual)
        or bundle_ref != MarketBundleRef.from_manifest(manifest)
    ):
        raise ValueError("overlay_stream_cover")
    return payload

def verify_binance_usdm_koru_directional_strategy_authority_v4(
    *, market_reader: MarketBundleReader
    ...
    try:
        ...
        compiler_ref, scope_ref, source_ref = (
            _artifact_ref(payload[key]) for key in ("compiler_result_ref", "scope_ref", "source_projection_authority_ref")
        )
        recipe_ref, strategy_ref, source_profile_ref = (
            _artifact_ref(payload[key]) for key in ("recipe_ref", "strategy_ref", "source_profile_authority_ref")
        )
        ...
        source_hash = _digest(payload["source_projection_authority_content_hash"], "source_projection_authority_content_hash")
        economics_ref = _market_ref(payload["economics_bundle_ref"])
        if (
            ...
            or compiler_ref != domain.ArtifactRef("koru_directional_target_compile_result", 2, compiler_digest)
            or scope_ref != domain.ArtifactRef("koru_directional_discovery_scope", 1, scope_digest)
            or source_ref != domain.ArtifactRef("binance_usdm_koru_tradifi_source_projection_authority_v3", 3, source_hash)
            or payload["compiler_source_projection_authority_ref"] != source_ref.to_canonical_dict()
            or payload["compiler_source_projection_authority_content_hash"] != source_hash
            ...
            return _failure("compiler_scope_source_binding")
        ...
        source_profile = _envelope(payload["source_profile_authority_envelope"])
        source_payload = source_profile.payload
        if (
            source_profile_ref != domain.ArtifactRef.from_envelope(source_profile)
            or source_profile.artifact_type != "binance_usdm_koru_source_profile_authority" or source_profile.schema_version != 3
            or not isinstance(source_payload, Mapping) or source_payload.get("type") != "binance_usdm_koru_source_profile_authority_v3"
            or source_payload.get("schema_version") != 3 or source_payload.get("source_fragment_digest") != source_fragment
            or source_payload.get("source_projection_authority_ref") != source_ref.to_canonical_dict()
        ):
            return _failure("source_profile_binding")
        target_key = _text(payload["target_stream_key"], "target_stream_key")
        target_events = _read(market_reader, target_key)
        stream_manifest = next(stream for stream in manifest.streams if stream.stream_key == target_key)
        if (
            stream_manifest.event_type != TARGET_STREAM_EVENT_TYPE or stream_manifest.capability != TARGET_STREAM_CAPABILITY
        ...
        target = PrecomputedTargetStream(target_key, target_events, 2)
        ...
        return KoruDirectionalV4StrategyAuthority(
            bundle_ref, bundle_ref.manifest_hash, economics_ref, _digest(payload["economics_bundle_digest"], "economics_bundle_digest"),
            _digest(payload["economics_authority_digest"], "economics_authority_digest"), compiler_ref, compiler_digest,
            _text(recipe["recipe_id"], "recipe_id"), _digest(payload["recipe_digest"], "recipe_digest"), strategy_ref, recipe_ref,
            _text(payload["strategy_id"], "strategy_id"), _text(payload["sleeve_id"], "sleeve_id"), target_key, digest,
            _text(recipe["target_exposure"], "target_exposure"), scope_ref, scope_digest, source_ref, source_hash, source_fragment,
            source_profile, source_profile_ref, target,
        )
