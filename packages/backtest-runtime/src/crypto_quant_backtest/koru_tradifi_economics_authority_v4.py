"""Target-free V4 economics admission for the KORU TradFi directional runtime."""
...
class KoruTradifiEconomicsAuthorityV4:
    """Verified V4 market, funding, fee, and margin inputs with no target."""
    ...
    def __post_init__(self) -> None:
        if (
            ...
            or type(self.source_projection_authority_ref) is not domain.ArtifactRef
            or self.source_projection_authority_ref != domain.ArtifactRef(
                "binance_usdm_koru_tradifi_source_projection_authority_v3", 3,
...
def _economics_payload(reader: LocalMarketBundleReader) -> tuple[Mapping[str, object], tuple[MarketEvent, ...], tuple[MarketEvent, ...]]:
    ...
    if any(stream.stream_key in _FORBIDDEN_ECONOMICS_STREAMS or ".target." in stream.stream_key for stream in manifest.streams if stream.stream_key != "binance_usdm.tradifi.target_overlay_authority.koruusdt.v4"):
        raise ValueError("legacy_economics_stream")
    return payload, events_by_key.get(_PRICE_STREAM, ()), events_by_key.get(_ACCOUNT_STREAM, ())


def resolve_koru_tradifi_economics_authority_v4(
    *, market_reader: MarketBundleReader, artifact_reader: ArtifactEnvelopeReader,
...
) -> KoruTradifiEconomicsAuthorityV4 | BinanceUsdmTradifiBarBacktestFailure:
    """Validate only V4 economics bytes and build their profile; targets are ignored."""
    try:
    ...
    try:
        ...
        if (
            ...
            or set(source_identity) != {"type", "schema_version", "source_projection_authority_ref", "source_projection_authority_content_hash", "source_fragment_digest", "source_projection_request_hash"}
            or source_identity.get("type") != "koru_tradifi_source_projection_content_identity_v3"
            or source_identity.get("schema_version") != 3
            or source_identity.get("source_fragment_digest") != payload["source_fragment_digest"]
            or _ref(source_identity.get("source_projection_authority_ref")) != domain.ArtifactRef("binance_usdm_koru_tradifi_source_projection_authority_v3", 3, source_identity.get("source_projection_authority_content_hash"))
        ...
        if (
            source_profile.artifact_type != "binance_usdm_koru_source_profile_authority" or source_profile.schema_version != 3
            or not isinstance(source_profile.payload, Mapping)
            or source_profile.payload.get("type") != "binance_usdm_koru_source_profile_authority_v3"
            or source_profile.payload.get("schema_version") != 3
            or source_profile.payload.get("source_fragment_digest") != payload["source_fragment_digest"]
            or source_profile.payload.get("source_projection_authority_ref") != source_identity.get("source_projection_authority_ref")
        ...
        if (
            ...
            or (extras and (len(extras) != 2 or "binance_usdm.tradifi.target_overlay_authority.koruusdt.v4" not in extras))
        ):
        ...
        projection_manifest = terms.get("projection_stream_manifest")
        projection_key = "binance_usdm.tradifi.bar_open.first_retained_aggregate_trade.koruusdt.1h.v3"
        if (
            not isinstance(projection_manifest, Mapping)
            or projection_manifest.get("stream_key") != projection_key
            or actual.get(projection_key) != projection_manifest
        ):
            raise ValueError("projection_manifest")
        ...
        if (
            ...
            or account.payload.get("schema_version") != _SCHEMA_VERSION
            or account.payload.get("operation_authorized") is not False
            or account.payload.get("order_authorized") is not False
            or account.payload.get("deployment_authorized") is not False
        ):
            raise ValueError("authority_events")
        source_bindings = terms.get("source_event_bindings")
        projection_bindings = terms.get("projection_event_bindings")
        if not isinstance(source_bindings, tuple) or not isinstance(projection_bindings, tuple):
            raise TypeError("source_bindings")
        source_keys = {stream["stream_key"] for stream in source_manifests}
        source_events = tuple(
            event for key in sorted(source_keys) if key != projection_key
            for event in _read(reader, key)
        )
        projection_events = _read(reader, projection_key)
        if (
            tuple(_binding(event) for event in source_events) != source_bindings
            or tuple(_binding(event) for event in projection_events) != projection_bindings
            or source_profile.payload.get("source_event_bindings") != tuple(
                {"stream_key": event.stream_key, "event_id": event.event_id, "event_hash": event.event_hash}
                for event in source_events
            )
            or source_profile.payload.get("execution_projection_stream_manifest") != projection_manifest
            or source_profile.payload.get("execution_projection_event_bindings") != tuple(
                sorted(
                    ({"stream_key": event.stream_key, "event_id": event.event_id, "event_hash": event.event_hash} for event in projection_events),
        ...
        result = built.result
        return KoruTradifiEconomicsAuthorityV4(
            reader.bundle_ref, payload["authority_digest"], refs[0],
            _ref(source_identity["source_projection_authority_ref"]),
            source_identity["source_projection_authority_content_hash"], payload["source_fragment_digest"],
        ...
        return _failure("economics_v4")


__all__ = ["KoruTradifiEconomicsAuthorityV4", "resolve_koru_tradifi_economics_authority_v4"]
