"""V4 directional execution planning over verified V4 economics."""
...
def _funding_stream_events(values) -> tuple[MarketEvent, ...]:
    if _FUNDING_STREAM not in {stream.stream_key for stream in values.market_bundle_manifest.streams}:
        return ()
    return _events(values, _FUNDING_STREAM)


def _funding_events(values, semantic_run_id: str):
    return _v2._funding_events(values, semantic_run_id) if _funding_stream_events(values) else ()


@dataclass(frozen=True, slots=True)
class BinanceUsdmTradifiDirectionalCasePlanningResultV4:
...
def _identity_plan_v4(
    values, rows: tuple[_V4CandidateRow, ...]
) -> tuple[ExecutionCaseIdentityRule, ...]:
    rules = [
        ExecutionCaseIdentityRule(
            "journal.initial.0",
            "binance-usdm-tradifi.directional-v4.initial-deposit",
            0,
        ...
        rules.extend(
            (
                ExecutionCaseIdentityRule(
                    ...
                    "binance-usdm-tradifi.directional-v4.order",
                    cycle_index,
                    domain.DomainIdKind.ORDER,
                ),
                ExecutionCaseIdentityRule(
                    f"fill.{execution_index}",
                    "binance-usdm-tradifi.directional-v4.fill",
                    execution_index,
                    domain.DomainIdKind.FILL,
                ),
                ExecutionCaseIdentityRule(
                    f"journal.fill.{execution_index}",
                    "binance-usdm-tradifi.directional-v4.fill-journal",
                    execution_index,
                    domain.DomainIdKind.JOURNAL,
                ),
                ExecutionCaseIdentityRule(
                    f"fee.{execution_index}",
                    "binance-usdm-tradifi.directional-v4.fee",
                    execution_index,
                    domain.DomainIdKind.FEE,
                ),
                ExecutionCaseIdentityRule(
                    f"journal.fee.{execution_index}",
                    "binance-usdm-tradifi.directional-v4.fee-journal",
                    execution_index,
                    domain.DomainIdKind.JOURNAL,
                ),
                ExecutionCaseIdentityRule(
                    f"order-event.fill.{execution_index}",
                    "binance-usdm-tradifi.directional-v4.order-event.fill",
                    execution_index,
                ),
                *(
                    ExecutionCaseIdentityRule(
                        f"order-event.{cycle_index}.0.{event_index}",
                        f"binance-usdm-tradifi.directional-v4.order-event.{event_type.value}",
    ...
    if versions <= {1}:
        ...
        raise ValueError("directional V4 funding model version")
    for index, event in enumerate(_funding_stream_events(values)):
...
def _preparation(values) -> MultiResolutionMarketDataPreparation:
    schedule = DecisionSchedule(
        "binance-usdm-tradifi.directional-v4.execution",
        3,
...
def _candidate_rows(values) -> tuple[_V4CandidateRow, ...]:
    ...
    for event in values.target_stream.events:
        ...
        if target is None:
            raise ValueError("directional V4 target exposure")
        ...
        if not isinstance(source_rows, tuple) or not all(isinstance(value, Mapping) for value in source_rows):
            raise ValueError("directional V4 source evidence")
        ...
        if (
            ...
            raise ValueError("directional V4 mark or next-boundary projection")
        _v2._decision_mark(values, mark, event)
        if projection is not None:
            _v2._execution_mark(values, projection)
        rows.append(_V4CandidateRow(event, candidate, target, mark, projection))
        prior_target = target
    if prior_target != 0:
        raise ValueError("directional V4 target stream must finish flat")
    if tuple((row.event.timeline_instant, row.event.event_id) for row in rows) != tuple(
        ...
        raise ValueError("directional V4 target decisions")
    return tuple(rows)


def _changed_rows(rows: tuple[_V4CandidateRow, ...]) -> tuple[_V4CandidateRow, ...]:
    previous = 0
    changed = []
    for row in rows:
...
@dataclass(frozen=True, slots=True)
class _V4EconomicExecutionAdapter:
    """V4 target authority paired with the established financial planning mechanics."""

    values: object
    rows: tuple[_V4CandidateRow, ...]
    ...
    def plan(self, semantic_run_id: str, journal_id: domain.DomainId) -> _ExecutionCasePlan:
        ...
        for cycle_index, row in enumerate(self.rows):
            ...
            if pending is not None and row.target != pending[0]:
                raise ValueError("directional V4 pending-target-conflict")
            ...
            if injected.injection is None:
                raise ValueError("directional V4 target did not pass standard validation")
            ...
            if rules is None:
                raise ValueError("directional V4 economics lacks order lattice")
            ...
            if allocated.allocation is None:
                raise ValueError("directional V4 full-sleeve allocation failed")
            if values.target.target_exposure == ".25" and row.target:
                ...
                if abs(target_notional.units) != 250_000_000 or target_notional.scale != _NOTIONAL_SCALE:
                    raise ValueError("directional V4 .25 target notional must be 2500.00000")
            ...
            if approved.approved_target is None:
                raise ValueError("directional V4 target risk evaluation failed")
            ...
            if sized.normalized_target is None:
                raise ValueError("directional V4 target sizing failed")
            ...
            if coordinated.decision is None or len(coordinated.decision.plan.planned_orders) != 1:
                raise ValueError("each directional V4 target change must produce one order")
            ...
            if row.projection is None:
                raise ValueError("directional V4 target change lacks next-boundary projection")
            ...
            if account is None:
                raise ValueError("directional V4 economics lacks fee authority")
...
def _plan(
    ...
    return _V4EconomicExecutionAdapter(values, rows, identity_plan).plan(
        semantic_run_id, journal_id
    )


def plan_binance_usdm_tradifi_directional_case_v4(values) -> BinanceUsdmTradifiDirectionalCasePlanningResultV4:
    """Plan V4 target changes with sealed V4 execution economics."""
    rows = _candidate_rows(values)
    identity_plan = _identity_plan_v4(values, rows)
    timeline_keys = tuple(sorted((
        values.target_stream_key, _PROJECTION_STREAM_PREFIX + "3", _LIQUIDATION_STREAM,
        *((_FUNDING_STREAM,) if _funding_stream_events(values) else ()),
    )))
    ...
    if type(timeline) is not DeterministicTimeline:
        raise ValueError("directional V4 overlay did not open a timeline")
    ...
    base = type(base)(
        base.schema_version,
        "binance-usdm-tradifi.directional-v4.execution-case",
        3,
        "binance-usdm-tradifi.directional-v4.execution",
        3,
    ...
    if outcome.resolved is None:
        raise ValueError("directional V4 request did not resolve")
    ...
    if _execution_case_semantic_spec_v3(base_spec=spec, execution_case_plan=actual_plan, market_data_preparation=preparation) != spec:
        raise ValueError("directional V4 identities changed semantics")
    ...
    return BinanceUsdmTradifiDirectionalCasePlanningResultV4(request, resolved, preparation, hydrated, case)


__all__ = ["BinanceUsdmTradifiDirectionalCasePlanningResultV4", "plan_binance_usdm_tradifi_directional_case_v4"]
