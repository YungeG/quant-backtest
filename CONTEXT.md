...
由有权机关发布、对明确适用范围和生效时点给出完整规则状态的可识别版本。

## 权威后继规则（Authoritative Successor）

由有权机关作出，并在其生效时改变、纠正、替代、废止、暂停或以其他方式取代既有官方规则修订的后续规则事实。

## 后继闭包（Successor Closure）

在明确的官方记录截止时点，对一个规则谱系的前序、后继关系和终端修订形成的有界、可审计结论。

## 目标范围投影（Target-scoped Projection）

把已闭合的规则谱系裁剪为一个明确有限目标区间，并保留其来源和推导身份的规则表示。

## 截止时点闭包与回溯闭包（As-of / Retrospective Closure）

截止时点闭包描述在声明截止时点可见的权威记录；回溯闭包使用目标结束后实际可用的证据，在更晚的记录截止时点确认目标期间的规则历史，且不把该证据倒记为更早可用。
