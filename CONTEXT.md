...
## 官方规则修订（Official Rule Revision）

由有权机关发布、对明确计算依据、适用范围和生效时点给出完整规则状态的可识别版本；同一官方行为可以有多个文献更正版本。

## 文献更正谱系（Documentary Correction Lineage）

同一官方行为的文献表示之间由更正关系形成的谱系；它决定在记录截止时点应采用哪个终端表示，但文献更正本身不等于经济规则后继。

## 经济后继谱系（Economic Succession Lineage）

不同官方行为的经济规则状态之间由前序关系和权威生效时点形成的谱系；其顺序不由发布时间或抓取时间决定。

## 权威后继规则（Authoritative Successor）

由有权机关作出，并在其权威生效时改变、替代、废止、暂停或以其他方式取代既有经济规则状态的后续规则事实。

## 后继闭包（Successor Closure）

在明确的官方记录截止时点，先为每个官方行为选择终端文献表示，再对经济后继状态形成无未决分叉、缺口、重叠或冲突的有界、可审计结论。

## 目标范围投影（Target-scoped Projection）

把已闭合的规则谱系按明确计算依据和完整适用范围裁剪为一个有限目标区间，并分别保留证据身份与目标经济身份的规则表示。

## 截止时点闭包与回溯闭包（As-of / Retrospective Closure）

截止时点闭包描述在声明官方记录截止时点已发布或登记的权威记录；回溯闭包使用目标结束后实际可用的抓取与收据证据，在不早于目标结束且不晚于证据可用时点的记录截止时点确认目标历史，不把证据倒记为历史 Profile 组合时点可用。
